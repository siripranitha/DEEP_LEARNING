...
class RNNLM_Model(nn.Module):
  ...
  def __init__(self, config):
    ...
    # biases with zero? wts with zero?
    # sigmoid for above layer?
  ...
  def add_embedding(self, input_x):
    ...
    for _step in range(self.config.num_steps):
      output.append(self.embedding_layer(input_x[:,_step]))
    ### END YOUR CODE
...
def compute_loss(outputs, y, criterion):
  ...
  _out = torch.stack(outputs, dim=1)
  _out = _out.flatten(start_dim=0,end_dim=1)
  #print(_out.shape,y.flatten().shape)
  # am i supposed to explode y as well?

  _y = y.flatten()
  loss = criterion(_out,_y)
